...
def take_category(dataset: TensorDataset, c: int):
    mask = dataset.tensors[1] <= c 
    return TensorDataset(dataset.tensors[0][mask], dataset.tensors[1][mask])
...
def load_dataset(dataset_name: str, loss: str) -> (TensorDataset, TensorDataset):
    ...
    if dataset_name == "mnist":
        ...
        train, test = load_cifar(loss)
        return take_first(train, 1000), take_first(test, 1000)
    elif dataset_name == "cifar10-1k-cat-1":
        train, test = load_cifar(loss)
        train = take_category(train, 2)
