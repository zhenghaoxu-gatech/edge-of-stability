...
for weight in ["w1_10_w2_40"]:
    for lr in lr_list_mse_relu_mnist[weight]: # must over 2/eta
        os.system(f"python src/gd.py mnist fc-relu-depth1  mse {lr} 200000 --neigs 1  --eig_freq 400 --width 200 --init_weight {weight}")
    
    for lr in lr_list_mse_cubicrelu_mnist[weight]:
        os.system(f"python src/gd.py mnist fc-cubic_relu-depth1  mse {lr} 200000 --neigs 1  --eig_freq 400 --width 200 --init_weight {weight}")  
    ...
    for lr in lr_list_mse_relu_BN_mnist[weight]:
        os.system(f"python src/gd.py mnist fc-relu-depth1  mse {lr} 200000 --batch_norm --neigs 1  --eig_freq 400 --width 200 --init_weight {weight}")  
